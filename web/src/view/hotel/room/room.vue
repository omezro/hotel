    ...
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="房型名称">
          <el-input v-model="searchInfo.name" placeholder="房型名称" />
        </el-form-item>
        <el-form-item>
...
const tableData = ref([])
const searchInfo = ref({ hotelId: Number(route.params.id) })

// 重置
const onReset = () => {
  searchInfo.value = { hotelId: Number(route.params.id) }
}
...
const getTableData = async() => {
  const table = await getSysRoomCostsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteSysRoomCostsByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateSysRoomCostsFunc = async(row) => {
  const res = await findSysRoomCosts({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteSysRoomCostsFunc = async(row) => {
  const res = await deleteSysRoomCosts({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
  formData.value.hotelId = Number(route.params.id)
  let res
  switch (type.value) {
    case 'create':
